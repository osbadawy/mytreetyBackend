...
if (!function_exists('filter_products')) {
    function filter_products($products)
    {
        ...
        if (get_setting('vendor_system_activation') == 1) {
            return $products->where('approved', '1')->where('published', '1')->where(function ($p) use ($verified_sellers) {
...
if (!function_exists('checkout_done')) {
    function checkout_done($combined_order_id, $payment)
    {
        ...
        if ($combined_order->type_id == 1) {
            $sus_requests = SustainabilityRequest::where('combined_order_id', $combined_order_id)->get();
            foreach ($sus_requests as $key => $sus_request) {
                $sus_request->status = 1;
                $sus_request->save();
            }
        } elseif ($combined_order->type_id == 2) {
            $giftcard = GiftCard::where('combined_order_id', $combined_order_id)->first();
            $giftcard->is_paid = 1;
            $giftcard->save();
            $giftcard_id = $giftcard->id;

            SendGiftCardJob::dispatch($giftcard_id)->delay(Carbon::parse($giftcard->delivary_date));
        } else {
            ...
            foreach ($combined_order->orders as $key => $order) {
                ...
                calculateCommissionPoints($order);
                } catch (\Exception $e) {
                }
            }
        }
...
if (!function_exists('calculateCommissionPoints')) {
    function calculateCommissionPoints($order)
    {
