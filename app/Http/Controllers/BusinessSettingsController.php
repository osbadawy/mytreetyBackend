...
class BusinessSettingsController extends Controller
{
    ...
    public function payment_method_update(Request $request): RedirectResponse
    {
     ...
     */
    public function google_analytics_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function google_recaptcha_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function google_map_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function google_firebase_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
     ...
     */
    public function facebook_chat_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function facebook_comment_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function facebook_pixel_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
     ...
     *
     */
    public function env_key_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
     ...
     * @return RedirectResponse
     */
    public function seller_verification_form_update(Request $request): RedirectResponse
    {
        $form = array();
    ...
    public function updateActivationSettings(Request $request): string
    {
    ...
    public function updateActivationSettingsInEnv($request): string
    {
    ...
    public function vendor_commission_update(Request $request): RedirectResponse
    {
        foreach ($request->types as $key => $type) {
            $business_settings = BusinessSetting::where('type', $type)->first();
            if ($business_settings == null) {
    ...
    public function shipping_configuration_update(Request $request): RedirectResponse
    {
        $business_settings = BusinessSetting::where('type', $request->type)->first();
